...
class FunSetSuite extends FunSuite {
  ...
  test("forall elements of set") {
    new TestSets {
      val s = forall(s5, (x=>x==2||x==1||x==3))
      val t = forall(s5, (x=>x==2||x==1))
      assert(s, "Forall 1")
      assert(!t, "Forall 2")
    }
  }

  test("exists an element of set") {
    new TestSets {
      val s = exists(s5, (x=>x==2))
      val t = exists(s5,(x=>x==0))
      assert(s, "Exists 1")
      assert(!t,"Exists 2")
    }
  }

  test("map an element of set") {
    new TestSets {
      val s = map(s4, (x=>x*x))
